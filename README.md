...
5. 작은 폼이 펼쳐짐:
   - `Branch`: `main` (그대로 둠)
   - `limit`: **`3`** 입력 (첫 테스트용)
   - `llm_mode`: **`auto`** 그대로 둠 (`limit` 테스트에서는 Gemma 설치를 건너뛰어 빠르게 확인)
6. 폼 하단 **녹색 `Run workflow` 버튼** 클릭

**3~10분 정도** 기다린 후 노션 DB로 가서 확인:
- 알림장 3개의 페이지 본문 OK
- 사진 OK
- 동영상/파일 첨부물 있으면 OK
...
테스트 3개가 정상이면, 같은 방법으로 다시 실행하되 이번에는 `limit`을 **비워둡니다** (전체 백업). `llm_mode=auto`는 전체 백업과 cron에서는 자동으로 Gemma를 사용합니다.
